...
class StorageManager:
    ...
    async def store_capture_result(self, result: CaptureResult) -> List[str]:
        ...
        metadata_date_dir = (
            self.metadata_dir / now.strftime("%Y") / now.strftime("%m") / now.strftime("%d")
        )
        ...
        try:
            ...
            for path in stored_paths:
                try:
                    ...
                    pass  # File cleanup failed, but we can continue
    ...
    async def _add_to_transfer_queue(self, image_paths: List[str], metadata_file: Path) -> None:
        ...
        for image_path in image_paths:
            ...
            queue_file = (
                self.transfer_queue_dir
                / f"transfer_{int(time.time() * 1000)}_{Path(image_path).stem}.json"
            )
    ...
    async def cleanup_old_files(self) -> Dict[str, Any]:
        ...
        cleanup_stats = {"files_removed": 0, "space_freed_mb": 0, "errors": 0}
            ...
            for queue_file in self.transfer_queue_dir.iterdir():
                if queue_file.is_file():
                    try:
                        ...
                        if (
                            queue_item.get("status") == "completed"
                            and queue_item.get("timestamp", 0) < cutoff_time
                        ):
                            queue_file.unlink()
                            cleanup_stats["files_removed"] += 1
                    except Exception as e:
    ...
    async def _check_available_space(self) -> Dict[str, float]:
        ...
        if buffer_size_gb > self.max_size_gb:
            logger.warning(
                f"Buffer size ({buffer_size_gb:.1f}GB) exceeds limit "
    ...
    async def mark_for_transfer(self, file_path: Path) -> None:
        ...
        queue_file = (
            self.transfer_queue_dir
            / f"manual_transfer_{int(time.time() * 1000)}_{file_path.stem}.json"
        )
        with open(queue_file, "w") as f:
        ...
        for queue_file in self.transfer_queue_dir.rglob("*.json"):
            if queue_file.is_file():
                try:
                    ...
                    if str(file_path) in queue_item.get("image_paths", []) or queue_item.get(
                        "file_path"
                    ) == str(file_path):
                        ...
                        updated_count += 1
                        logger.info(
                            f"Marked transfer {'complete' if success else 'failed'}: {file_path}"
                        )
                except Exception as e:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        pending_transfers = len(
            [item for item in transfer_queue if item.get("status") == "pending"]
        )
        completed_transfers = len(
            [item for item in transfer_queue if item.get("status") == "completed"]
        )
        failed_transfers = len([item for item in transfer_queue if item.get("status") == "failed"])

        # Calculate total files across all types
        total_files = (
            usage_info["by_type"]["images"]["count"]
        ...
        return {
            ...
            "activity": {
                ...
                "average_file_size_mb": (
                    usage_info["by_type"]["images"]["size_mb"]
