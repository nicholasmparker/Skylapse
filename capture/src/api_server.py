...
class CaptureAPIServer:
    ...
    async def _get_metrics(self, request) -> web.Response:
        ...
        try:
            ...
            # Camera metrics
            camera_status = await self.controller._camera_controller.get_camera_status()
            metrics.append(f"skylapse_camera_initialized {1 if camera_status['initialized'] else 0}")
            ...
            if 'performance' in camera_status:
                ...
                metrics.append(f"skylapse_capture_time_ms_avg {perf['average_capture_time_ms']:.2f}")
