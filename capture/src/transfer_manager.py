...
class TransferManager:
    ...
    def _update_average_transfer_time(self, transfer_time_ms: float) -> None:
        ...
        if completed_count > 0:
            self._transfer_stats['average_transfer_time_ms'] = (
                (current_avg * (completed_count - 1) + transfer_time_ms) / completed_count
            )
        ...
        for manifest_file in self._queue_dir.glob("transfer_*.json"):
            try:
                ...
                if (manifest.get('status') in ['completed', 'failed'] and
                    manifest.get('created_at', 0) < cutoff_time):
    ...
    async def _execute_rsync_command(self, source_files: List[str], target_path: str) -> Dict[str, Any]:
