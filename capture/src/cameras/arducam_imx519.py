...
class ArducamIMX519Camera(CameraInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._capture_timeout_ms = self._capture_config.get('capture_timeout_ms', 10000)  # Increased to 10s
    ...
    async def get_preview_frame(self) -> Optional[bytes]:
        ...
        try:
            ...
            cmd = [
                self._rpicam_cmd,
                '-t', '100',  # Very short capture time
                '-n',  # No preview window
                '--width', '640',  # Small preview size
                '--height', '480',
                '-o', temp_path
            ]
    ...
    async def autofocus(self, timeout_ms: int = 2000) -> bool:
        ...
        try:
            ...
            cmd = [
                self._rpicam_cmd,
                '-t', '1000',  # 1 second for focus
                '-n',  # No preview
                '--width', '640',  # Small test image
                '--height', '480',
                '-o', temp_path
            ]
    ...
    async def _build_capture_command(self, settings: CaptureSettings, output_path: str) -> List[str]:
