...
class JobQueue:
    ...
    async def get_next_job(self) -> Optional[Dict[str, Any]]:
        ...
        self._pending_jobs.sort(key=lambda job_id: (
    ...
    def _update_stats(self) -> None:
        ...
        self._stats = {
            ...
            'completed_jobs': len([j for j in self._jobs.values() if j['status'] == JobStatus.COMPLETED.value]),
            'failed_jobs': len([j for j in self._jobs.values() if j['status'] == JobStatus.FAILED.value]),
            'pending_jobs': len([j for j in self._jobs.values() if j['status'] == JobStatus.PENDING.value]),
            'in_progress_jobs': len([j for j in self._jobs.values() if j['status'] == JobStatus.IN_PROGRESS.value])
        }
