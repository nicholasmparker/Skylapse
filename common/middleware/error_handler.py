                        ...
                        class RequestWithBody:
                            ...
                            async def json(self):
                                if self._json_cache is None:
                                    parsed_json = json.loads(self._body.decode("utf-8"))
                                    # Ensure we always return a dict for valid JSON requests
                                    # If the JSON is a primitive type (string, number, etc),
                                    # wrap it in a dict to maintain API contract expectations
                                    if not isinstance(parsed_json, dict):
                                        self._json_cache = {"value": parsed_json}
                                    else:
                                        self._json_cache = parsed_json
